...
describe("CreatedDateAttributeRenderer", () => {
  it("renders an empty attribute value when value is null", () => {
    render(<CreatedDateAttributeRenderer value={null} />);
    expect(screen.getByText("—")).toBeInTheDocument();
  });

  it("renders the raw value when it cannot be parsed as a date", () => {
    render(<CreatedDateAttributeRenderer value="not-a-date" />);
    expect(screen.getByText("not-a-date")).toBeInTheDocument();
  });

  it("prefixes the formatted date with 'created'", () => {
    const value = "2016-06-21T00:00:00.000Z";
    render(<CreatedDateAttributeRenderer value={value} />);
