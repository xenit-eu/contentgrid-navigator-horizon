...
- Resolve templates with `resolveTemplate` from `@contentgrid/hal-forms`.
  When the template is present, a
  `HalFormsTemplate<TypedRequestSpec<void, HalShape>>` is returned; when it
  is not present, `null` is returned (absence = operation not permitted,
  see rule 2).
- When a template was found, execute it by constructing values with
  `createValues` from `@contentgrid/hal-forms/values`:

  ```tsx
  // Importing HAL Forms values utilities
  import { HalFormValues, createValues } from "@contentgrid/hal-forms/values";
  ...
  const handleChange = (
    newFormData: HalFormValues<TypedRequestSpec<EntityInstanceForUpdate, EntityInstanceShape>>,
  ) => {
...
- HAL-Forms codecs are used to encode form values into proper API requests:
...
- Form state management workflow:
  ...
  3. Encode form values to a request using HAL-Forms codecs
...
## HAL-FORMS search template guide

How to use `@contentgrid/hal-forms` to work with **search templates**.
HAL-Forms provides a standardized way to describe and interact with hypermedia
forms, enabling type-safe, schema-driven API requests. Search operations MUST
be executed through the `search` template present on the collection resource
or the entity profile — never by hand-building query URLs.

A HAL-Forms template (`HalFormsTemplate`) describes:

- **Properties**: available fields and their types
...
A template is typically obtained from a HAL resource:

```tsx
import { HalFormsTemplate } from "@contentgrid/hal-forms";
import { SearchRequestSpec } from "../app/api/requests";

// The template defines the search form structure
const searchTemplate: HalFormsTemplate<SearchRequestSpec> = profile.searchTemplate;
```

### 2. Create values from the template

Use `createValues()` to initialize an empty values object:

```tsx
import { createValues } from "@contentgrid/hal-forms/values";

// Create empty values conforming to the template
const values = createValues(searchTemplate);
```

This returns a `HalFormValues<SearchRequestSpec>` object that:

- is **immutable** (use `withValue()` to update),
- tracks which properties have values,
- validates against the template schema.

### 3. Set values (immutable updates)

Use `withValue()` to add or update field values:

```tsx
// Set a single value
const v1 = values.withValue("name", "John");

// Chain multiple updates
const v2 = values.withValue("name", "John").withValue("last_name", "Doe");

// Remove a value
const v3 = values.withoutValue("name");
```

Important: `withValue()` returns a **new** values object; the original is
unchanged:

```tsx
const v1 = createValues(template);
const v2 = v1.withValue("name", "test");

console.log(v1 === v2); // false - different objects
console.log(v1.hasValue("name")); // false - v1 is unchanged
console.log(v2.hasValue("name")); // true - v2 has the value
```

### 4. Encode values to an API request

Use HAL-Forms codecs to convert values into an actual API request. The codec
automatically handles encoding based on the template's `method` and
`contentType` (e.g. GET → query parameters with
`application/x-www-form-urlencoded`, POST → JSON body):

```tsx
import halFormCodecs from "@contentgrid/hal-forms/codecs";

// Get the appropriate codec for your template
const codec = halFormCodecs.requireCodecFor(searchTemplate);

// Encode values into a request object
const request = codec.encode(values);

// request now contains:
// {
//     method: "GET",
//     url: "/api/entities/search?name=report&status=published&createdAfter=2024-01-01",
//     headers: { "Content-Type": "application/x-www-form-urlencoded" },
//     body: null  // For GET requests
// }
```

Encoding may throw when values do not validate against the template — handle
errors at the call site.

### Working with template properties

Templates expose their properties through the `properties` array:

```tsx
searchTemplate.properties.forEach((property) => {
  console.log(property.name); // e.g., "name", "status"
  console.log(property.type); // e.g., "text", "select"
  console.log(property.prompt); // e.g., "Document Name"
  console.log(property.required); // true/false
  console.log(property.value); // default value
  console.log(property.options); // for select fields
});
```

### Accessing values

Read values from a `HalFormValues` object:

```tsx
const values = createValues(searchTemplate)
  .withValue("name", "report")
  .withValue("status", "draft");

// Get a specific value
const nameValue = values.value("name"); // "report"

// Check if a value exists
const hasStatus = values.hasValue("status"); // true

// Get all value entries
values.entries().forEach(([propertyName, value]) => {
...
When using TypeScript, HAL-Forms provides type checking:

```tsx
import { HalFormValues } from "@contentgrid/hal-forms/values";
import { SearchRequestSpec } from "../app/api/requests";

// values is typed to SearchRequestSpec
const values: HalFormValues<SearchRequestSpec> = createValues(searchTemplate);

// TypeScript knows what fields are available
values.withValue("name", "test"); // ✓ valid
values.withValue("invalid", "test"); // ✗ type error (if strict typing is enabled)
```

### Best practices

1. **Always use `createValues()`** to initialize values from a template
2. **Never mutate** values objects directly — always use `withValue()` and
   `withoutValue()`
3. **Validate templates exist** before using them (see affordance rule 2)
4. **Use codecs** for encoding — don't build URLs manually (see affordance
   rule 3)
5. **Handle errors** when encoding (validation may fail)

Related entry points:

- `@contentgrid/hal` — HAL resource navigation
- `@contentgrid/hal-forms` — form templates and `resolveTemplate`
- `@contentgrid/hal-forms/values` — value manipulation utilities
- `@contentgrid/hal-forms/codecs` — request encoding/decoding
