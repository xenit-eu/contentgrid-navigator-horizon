...
describe("NumberRenderer", () => {
  it("renders a numeric input with the given value", () => {
    render(<NumberRenderer {...numberField()} value={5} onChange={vi.fn()} />);
    expect(screen.getByRole("spinbutton")).toHaveValue(5);
  });

  it("renders an empty input for an empty-string value", () => {
    render(<NumberRenderer {...numberField()} value="" onChange={vi.fn()} />);
    expect(screen.getByRole("spinbutton")).toHaveValue(null);
  });

  it("calls onChange with a coerced number", () => {
    const onChange = vi.fn();
    render(<NumberRenderer {...numberField()} value="" onChange={onChange} />);
  ...
  it("calls onChange with an empty string when cleared", () => {
    const onChange = vi.fn();
    render(<NumberRenderer {...numberField()} value={7} onChange={onChange} />);
