...
- **Search operations** must be executed through the `search` template present on the collection resource or the entity profile — never by hand-building query URLs. See the search template guide in [`packages/navigator-data/CLAUDE.md`](packages/navigator-data/CLAUDE.md).
