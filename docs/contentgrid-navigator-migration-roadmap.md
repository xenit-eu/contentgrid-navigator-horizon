...
**Estimation basis:** AI-driven implementation (Claude Code / agents writing the bulk of code; human in review + decision loop). See _Estimation notes_ at end.
...
- Estimates exclude meetings, context switches, and the cost of deciding _what_ to build — only execution.
...
| 3.3 | Build "patterns" layer: `EntityCard`, `DataTable`, `FilterSidebar`, `RelationSection`, `EntityItemPicker`, `FileUploadZone`, `BrandingHeader` — extracted from `apps/navigator`                                                                                                       | 1.5d     | M          |
...
Promotes the data/auth/config layer to a **workspace package** that the generic and experimental apps consume via `workspace:*`. Publish ceremony is deferred — see _HAL adapter — Release model_ for triggers.
...
| #    | Task                                                                                                                                                                                                                                                                                                                                                                                                                            | Estimate | Confidence |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------- |
| 5A.1 | Bridge `@contentgrid/hal-forms.HalFormsTemplate` → `FieldDescriptor[]` via `resolveCreateFieldDescriptors` in `packages/features/src/entity-item-create/model/` (per [ADR-004](adr/ADR-004-drop-jsonforms-halforms-renderer.md), superseding this row's original `RenderFieldDescriptor[]` / `packages/navigator-data/src/form-fields/` plan). Handles type mapping, range-pair pairing, options/enum, relation-link references | 0.75d    | M          |
| 5A.2 | Replace `useFormFields` with `useEntityFormState` to consume `_templates.create-form` / `default-form` via the bridge                                                                                                                                                                                                                                                                                                           | 0.5d     | H          |
...
- **Spikes go in the sprint _before_ their dependent phase.** 5A bridge validation during Phase 3; 6B.1 (extraction behaviour spec) + 6B.2 (PDF coord-system reconciliation) during Phase 3. Their findings drive the ADR-011 fallback decision _before_ Phase 6A starts.
...
| Agent-generated tests are tautological / low-value                                          | M          | M      | Human review of test PRs; require coverage of _behaviour_ not _implementation_                                                                                 |
